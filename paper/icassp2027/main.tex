...
remain participant-disjoint from our training split. \textbf{HeAR} is added post hoc on
UKCOVID and in the external analyses as a third frozen health-acoustic representation
\cite{baur2024hear}; it is not promoted to a primary UKCOVID backbone.
...
\begin{figure*}[t]
  ...
  \caption{\dpair{} estimates with 95\% paired bootstrap intervals. The 12
  settings comprise UKCOVID (primary: AST, OPERA; post-hoc: HeAR) and three external
  sensitivities (AST, OPERA, HeAR each). Retrieval CIs excluded zero in 11/12 settings and
  sex-probe CIs in 12/12, whereas no matched disease CI established a positive gain. Panel scales differ;
