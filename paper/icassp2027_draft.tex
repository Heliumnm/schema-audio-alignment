...
(iii) a probe decomposition showing genuine participant mapping without measurable
matched-population disease benefit.
...
waveform QC, decoded-PCM duplicate removal, patient-disjoint development splits, and 1:1
covariate matching. Formal execution requires at least 100 balanced pairs and adequate
development class counts. A metadata-only capacity audit provides a ceiling of 114 pairs
and first passes deterministic balance trimming at 101; waveform QC can therefore close
the gate. If it passes, the same pairing arms and raw, artefact, metadata, and direct-fusion
baselines are read once. UKCOVID and Coswara are not pooled. This is an external
patient-level stress test, never confirmatory proof.
...
\begin{figure*}[t]
  ...
  \caption{Correct-minus-within-label $\Delta$AUROC (left) and $\deltanll$ (right) across
  Standard, matched, and matched-long. Any later Coswara estimate is displayed separately
  and never pooled.}
  \label{fig:paired}
\end{figure*}
...
patient-level stress-test estimate if the 100-pair gate passes, or a transparent NO-GO.
...
and the pending Coswara cohort is near its minimum gate. Future work should use untouched
cohorts, additional backbones and readouts, and separate audible evidence from independent
clinical context.
