"""Audit all frozen CODA epoch-500 projector artefacts before test evaluation.

This script deliberately computes no retrieval, probe, TB, validation, or test score.  It
only verifies that the 4 arms x 5 seeds x 2 backbones are complete, internally consistent,
and still implement the pairings frozen in the preregistration.
"""
...
def execute(config_file: str) -> Path:
    ...
    expected_runs = {f"{arm}_seed{seed}" for arm in ARMS for seed in SEEDS}
    failures: list[str] = []
    output = {
        "format_version": "coda-formal-training-audit-v1",
    ...
    pairing_hashes: dict[str, dict[str, str]] = {}

    for backbone in BACKBONES:
        directory = root / "models" / backbone / "alignment"
        manifest_path = directory / "manifest.json"
        if not manifest_path.is_file():
            failures.append(f"{backbone}:manifest_missing")
            continue
        manifest = json.loads(manifest_path.read_text())
        runs = manifest.get("runs", {})
        if set(runs) != expected_runs:
            failures.append(f"{backbone}:wrong_run_set")
        ...
        if int(manifest.get("n_train", -1)) != len(train) or int(manifest.get("batch", -1)) != 64:
            failures.append(f"{backbone}:wrong_train_or_batch_count")

        arm_records = {}
        pairing_hashes[backbone] = {}
        for seed in SEEDS:
            ...
            if len(init) != 1 or None in init:
                failures.append(f"{backbone}:seed{seed}:initialisation_not_shared")
            if len(order) != 1 or None in order:
                failures.append(f"{backbone}:seed{seed}:batch_order_not_shared")
        ...
        for arm in ARMS:
            for seed in SEEDS:
                ...
                if not archive_path.is_file() or not checkpoint_path.is_file():
                    failures.append(f"{backbone}:{key}:missing_file")
                    continue
                archive = np.load(archive_path, allow_pickle=True)
                required = {"participants", "raw", "normalized", "pairing", "seed", "arm",
                            "epochs", "input_hashes", "source_commit"}
                if not required <= set(archive.files):
                    failures.append(f"{backbone}:{key}:archive_keys")
                    continue
                ...
                if not legal:
                    failures.append(f"{backbone}:{key}:invalid_representation")
                if pairing.shape != (len(train),) or not np.array_equal(
                        np.sort(pairing), np.arange(len(train))):
                    failures.append(f"{backbone}:{key}:pairing_not_bijection")
                else:
                    fixed = pairing == np.arange(len(train))
                    if arm == "correct" and not np.all(fixed):
                        failures.append(f"{backbone}:{key}:correct_not_identity")
                    if arm != "correct" and np.any(fixed):
                        failures.append(f"{backbone}:{key}:shuffle_fixed_point")
                    if arm in ("within_label", "within_label_sex") and not np.all(
                            y[pairing] == y):
                        failures.append(f"{backbone}:{key}:within_crosses_label")
                    if arm == "within_label_sex" and not np.all(sex[pairing] == sex):
                        failures.append(f"{backbone}:{key}:within_sex_crosses_sex")
                ...
                if raw_hash != run.get("repr_raw_hash") or norm_hash != run.get("repr_norm_hash"):
                    failures.append(f"{backbone}:{key}:representation_hash")
                if pair_hash != run.get("pairing_hash"):
                    failures.append(f"{backbone}:{key}:pairing_hash")
                if int(run.get("n_repr", -1)) != len(table):
                    failures.append(f"{backbone}:{key}:wrong_n_repr")
                if not float(run.get("loss_last", np.inf)) < float(run.get("loss_first", -np.inf)):
                    failures.append(f"{backbone}:{key}:loss_not_decreased")
                pairing_hashes[backbone][key] = pair_hash
                arm_records[key] = {
                    "archive_sha16": raw_hash,
        ...
        output["backbones"][backbone] = {
            "manifest_runs": int(len(runs)),
            "schedule": {"arms": list(ARMS), "seeds": list(SEEDS), "epochs": 500},
            "runs": arm_records,
        }

    if set(pairing_hashes) == set(BACKBONES):
        for key in expected_runs:
            if pairing_hashes["ast"].get(key) != pairing_hashes["opera_ct"].get(key):
                failures.append(f"cross_backbone:{key}:pairing_differs")
    output["technical_pass"] = not failures
    output["failures"] = failures
    destination = root / "public" / "formal_training_audit.json"
    atomic_json(destination, output)
    print(f"FORMAL TRAINING {'PASS' if not failures else 'FAIL'}: {destination}")
    if failures:
        raise SystemExit(3)
    return destination
